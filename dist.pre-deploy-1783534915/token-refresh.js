...
const BASE_BACKOFF_MS = 30000; // first retry ~30s, then ~60s (exponential)
...
function backoffMs(retry, base, rng) {
...
async function refreshAgentOnce(agent, fetchImpl) {
    try {
...
async function refreshAgent(agent, opts = {}) {
    ...
    let last = { ok: false, retriable: true, reason: "no attempt made" };
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (attempt < maxAttempts) {
            ...
            log.warn(`Token refresh attempt ${attempt}/${maxAttempts} failed for ${agent.name}: ${last.reason}. Retrying in ${Math.round(wait / 1000)}s...`);
            await sleep(wait);
        }
    }
    ...
    log.error(`Token refresh exhausted all ${maxAttempts} attempts for ${agent.name}: ${last.ok ? "" : last.reason}`);
    notify({
        severity: "critical",
