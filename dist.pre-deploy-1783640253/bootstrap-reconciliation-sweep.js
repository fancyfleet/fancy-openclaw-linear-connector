 ...
 * filter.
 */
async function queryUnenrolledTickets(authToken, fetchFn) {
    const query = `
 ...
 */
export async function runBootstrapReconciliationSweep(opts) {
    ...
    for (const ticket of candidates) {
        ...
        try {
            ...
            if (issue.labels.some((l) => l.name.startsWith("state:")))
                continue;
            const bootstrapResult = await applyBootstrapToIssue(issue, authToken, opts.workflowRegistry);
            if (bootstrapResult?.action === "bootstrapped") {
                result.healed++;
                log.info(`bootstrap-reconciliation: healed ${ticket.identifier} → ${bootstrapResult.workflowId}:${bootstrapResult.entryState}`);
                // Dispatch wake to the first-owner delegate
                if (wakeFn) {
                    try {
                        await wakeFn(bootstrapResult.delegateAgentName ?? "", bootstrapResult.ticketIdentifier ?? ticket.identifier);
                    }
                    catch (wakeErr) {
 ...
 * loops.
 */
export function registerBootstrapReconciliationCron(opts) {
    const intervalMs = opts.intervalMs ?? DEFAULT_INTERVAL_MS;
    registerCron("bootstrap-reconciliation-sweep", `every ${formatIntervalMs(intervalMs)}`);
    if (!opts.authToken) {
        log.warn("bootstrap-reconciliation: no auth token provided — sweep will be skipped until the next call");
    }
    const timer = setInterval(() => {
        ...
        void runBootstrapReconciliationSweep({
            ...
            log.error(`bootstrap-reconciliation: unexpected sweep failure: ${err instanceof Error ? err.message : String(err)}`);
        });
    }, intervalMs);
    timer.unref();
...
//# sourceMappingURL=bootstrap-reconciliation-sweep.js.map