...
import { loadUniversalCanon, formatCanonBlock, getActiveCanonVersion } from "../policy/universal-canon.js";
...
export async function sendWakeUpSignal(agentId, ticketIds, config) {
    ...
    try {
        const deliverOnce = () => deliverMessageToAgent(agentId, sessionKey, message, config);
        ...
        if (config.deliveryScheduler) {
            ...
            if (outcome.status !== "delivered") {
                throw new Error(`wake-up delivery undeliverable after ${outcome.attempts} attempt(s)`);
            }
