...
export class MutationAuditStore {
    ...
    `).run(input.recordedAt ?? new Date().toISOString(), input.source, input.ticket, input.changeType, input.field ?? null, input.oldValue ?? null, input.newValue ?? null, input.actorId ?? null, input.agent ?? null, input.intent ?? null, input.webhookEventId ?? null, input.opName ?? null, input.ticketUuid ?? null);
        this.writeCount++;
    ...
    `);
        ...
        const tx = this.db.transaction(() => {
            for (const input of inputs) {
                const result = insert.run(input.recordedAt ?? new Date().toISOString(), input.source, input.ticket, input.changeType, input.field ?? null, input.oldValue ?? null, input.newValue ?? null, input.actorId ?? null, input.agent ?? null, input.intent ?? null, input.webhookEventId ?? null, input.opName ?? null, input.ticketUuid ?? null);
