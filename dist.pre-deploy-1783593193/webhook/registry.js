...
function writeMeta(envFile, meta) {
...
function persistSecrets(envFile, secrets) {
...
export function addWebhook(input) {
    ...
    if (!secret)
        return { ok: false, status: 400, error: "A signing secret is required." };
    if (!url)
        return { ok: false, status: 400, error: "A webhook URL is required." };
    let parsed;
    try {
        parsed = new URL(url);
    }
