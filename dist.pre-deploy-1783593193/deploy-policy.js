 ...
 */
export function loadDeployPolicy() {
    ...
    if (cache && cache.path === file && cache.mtimeMs === mtimeMs)
        return cache.policy;
    try {
 ...
 */
function repoMatchesKey(repoRef, key) {
    ...
    if (!k.includes("/") && refName === keyName)
        return true; // bare key ↔ qualified ref
    if (!ref.includes("/") && refName === keyName)
        return true; // bare ref ↔ qualified key
    return false;
}
/**
 ...
 */
export function reposWithoutCiAutoDeploy(repoRefs) {
...
//# sourceMappingURL=deploy-policy.js.map