 ...
 */
import { getAgents, onAgentsReloaded } from "./agents.js";
...
let _status = { lastCheck: null, violations: [], notes: [] };
/** Physical (docker) container an agent runs in, derived from its secretsPath. */
export function physicalContainerOf(agent) {
    const match = (agent.secretsPath ?? "").match(/\/containers\/([^/]+)\/workspace\//);
    return match ? match[1] : null;
}
function findAgentForBody(body, agents) {
    const candidates = [body.id, body.openclaw_agent].filter(Boolean);
    return agents.find((a) => candidates.includes(a.name) || (a.openclawAgent !== undefined && candidates.includes(a.openclawAgent)));
}
/**
 * Pure cross-check. Returns violations (loud) and notes (informational).
 * Exported for tests.
 */
export function crossCheckRegistryPolicy(agents, bodies) {
    const violations = [];
    const notes = [];
    for (const body of bodies) {
        const agent = findAgentForBody(body, agents);
        if (!agent) {
            violations.push(`policy body '${body.id}' has no registered agent in agents.json — ` +
        ...
        if (declared !== physical) {
            violations.push(`policy body '${body.id}' places the agent in '${declared}' but the registry says '${physical}' ` +
    ...
    const bodiless = agents.filter((a) => !bodies.some((b) => {
        const ids = [b.id, b.openclaw_agent].filter(Boolean);
        return ids.includes(a.name) || (a.openclawAgent !== undefined && ids.includes(a.openclawAgent));
    }));
    if (bodiless.length > 0) {
        notes.push(`registered agents with no policy body (fail-closed, no workflow capabilities): ` +
 ...
 */
export async function runRegistryPolicyCheck(trigger) {
    try {
