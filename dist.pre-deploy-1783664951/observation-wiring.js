...
function emptySkipCounts() {
    return Object.fromEntries(OBSERVATION_SKIP_REASONS.map((r) => [r, 0]));
}
let _store;
let _counters = {
    appended: 0,
 ...
 */
export function registerObservationWriter(store) {
    _store = store;
    // AC1.6: liveness is observable from the startup log alone, before any
    // feedback-required transition has occurred.
    log.info(`observation write path registered at bootstrap: db=${store.dbPath} rows=${store.count()}`);
    return store;
}
/**
 ...
 */
export function getRegisteredObservationStore() {
    return _store;
}
/** Record a successful append. `degraded` marks an `unspecified` reason code. */
export function countObservationAppended(degraded) {
    _counters.appended += 1;
    if (degraded)
        _counters.degraded += 1;
}
/** Record a skipped observation write. */
export function countObservationSkip(reason) {
    _counters.skipped += 1;
 ...
 */
export function getObservationLiveness() {
    let rows = null;
    if (_store) {
        try {
            rows = _store.count();
...
export function resetObservationWiring() {
    _store = undefined;
    _counters = { appended: 0, degraded: 0, skipped: 0, skipsByReason: emptySkipCounts() };
}
//# sourceMappingURL=observation-wiring.js.map