 ...
 */
function withCanonBlock(message, canon) {
    if (!canon)
        return message;
...
async function buildDelegationMessage(reason, identifier, title, authToken) {
    // AI-2044: mention-derived wakes must not claim ownership. "You were
    // assigned" on a mention route made bystanders believe they held tickets
    // they did not (AI-2040: Igor woken as owner of a ticket with delegate null).
    const actionText = reason === "delegate"
        ? `You were delegated ${identifier}`
        : reason === "mention" || reason === "body-mention"
            ? `You were mentioned on ${identifier} — you are not necessarily its delegate; read the ticket before taking ownership actions`
