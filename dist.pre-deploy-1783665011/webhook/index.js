...
import { routeEventAll, unresolvedRoutingCandidates } from "../router.js";
...
import { emitDelegateUnavailable, emitOrphanedGovernedTicket } from "../escalation.js";
import { checkRoleGuardAndBlock, fetchCurrentDelegate } from "../routing-guard.js";
 ...
 */
export async function enrichCommentEventForRouting(event) {
    if (event.type !== "Comment")
        return;
    const data = event.data;
    if (!data || data.delegate || data.assignee)
        return;
    ...
    try {
        ...
        if (issue?.delegate?.id)
            data.delegate = { id: issue.delegate.id };
        ...
        if (issue?.identifier && !data.identifier && !data.issueIdentifier)
...
export function createWebhookRouter(eventStore, nudgeStore, agentQueue, bag, sessionTracker, throttle, operationalEventStore, onDispatched, onAgentActivity, onDeliveryCommitted, enrolledTicketsStore, mutationAuditStore, idempotencyStore) {
    const router = Router();
    ...
    router.post("/", async (req, res) => {
        ...
        if (routes.length === 0) {
            ...
            const noRouteIssue = noRouteData?.issue;
            const noRouteTicket = noRouteData?.identifier ||
                noRouteData?.issueIdentifier ||
                noRouteIssue?.identifier ||
                null;
            const noRouteRawId = noRouteData?.issueId || noRouteData?.id || null;
            ...
            appendOperationalEvent(operationalEventStore, {
                ...
                errorSummary: `No agent target for ${event.type}${attribution}` +
                    (humans.length > 0 ? ` — known human: ${humans.map((id) => knownHumans.get(id)).join(", ")}` : ""),
            });
            if (humans.length > 0) {
                log.info(`no-route candidates resolved to known human(s): ${humans.map((id) => `${knownHumans.get(id)} (${id})`).join(", ")}` +
        ...
        async function dispatchRoute(route) {
            ...
            if (idempotencyStore) {
                ...
                const idempotencyResult = idempotencyStore.checkAndRecord(ticketId, stateName, route.agentId, updatedAt);
            ...
            if (roleGuardToken) {
                try {
                    ...
                    // ── AI-2044 (AC 4): mention routes never carry ownership on a
                    // governed ticket. A mention/body-mention dispatch may only wake the
                    // agent who IS the current delegate; anyone else is a bystander named
                    // in prose. A null-delegate governed ticket in a working state is an
                    // orphan — raise a loud steward escalation instead of falling
                    // through to the most recent commenter/mention (AI-2040, AI-2044).
                    const isGoverned = guardLabels.some((l) => /^wf:/i.test(l));
                    const isMentionRoute = route.routingReason === "mention" || route.routingReason === "body-mention";
                    if (isGoverned && isMentionRoute) {
                        const guardAuthHeader = /^Bearer\s+/i.test(roleGuardToken) ? roleGuardToken : `Bearer ${roleGuardToken}`;
                        const delegateFetch = await fetchCurrentDelegate(issueIdentifier, guardAuthHeader);
                        const targetLinearId = linearUserIdResolver(route.agentId);
                        if (!delegateFetch.ok || delegateFetch.delegateId !== targetLinearId || !targetLinearId) {
                            const hasWorkingState = guardLabels.some((l) => /^state:/i.test(l) && !/^state:(done|escape)$/i.test(l));
                            const orphaned = delegateFetch.ok && delegateFetch.delegateId === null && hasWorkingState;
                            log.warn(`AI-2044: suppressed mention-route ownership dispatch to ${route.agentId} [${issueIdentifier}] — ` +
                                (orphaned ? "governed ticket has NO delegate (orphaned)" : "target is not the current delegate"));
                            appendOperationalEvent(operationalEventStore, {
                                ...
                                key: ticketId,
                                sessionKey: ticketId,
                                deliveryMode: orphaned ? "orphaned-governed-ticket" : "mention-nondelegate-suppressed",
                            });
                            if (orphaned) {
                                await emitOrphanedGovernedTicket(issueIdentifier, `mention-route fallthrough to ${route.agentId} suppressed`, roleGuardToken);
                            }
                            nudgeStore?.clearNudge(route.agentId, ticketId);
                            return;
                        }
            ...
            try {
                ...
                if (directResult.runId && onDispatched)
                    onDispatched(agentName, ticketId);
            }
            catch (err) {
