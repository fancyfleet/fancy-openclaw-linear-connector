...
export function matrixMessageTransport() {
    ...
    return {
        ...
        send: async (message) => {
            const args = { action: "send", channel: "matrix", target: room, message };
            if (account)
                args.accountId = account;
            const response = await postJson(`${gatewayUrl}/tools/invoke`, token, { tool: "message", args });
            const text = await response.text();
            if (!response.ok)
                throw new Error(`tools/invoke message returned ${response.status}: ${text.slice(0, 200)}`);
            const parsed = JSON.parse(text);
            if (parsed.ok === false)
                throw new Error(`message tool failed: ${parsed.error?.message ?? "unknown"}`);
        },
    };
}
export function hookRelayTransport() {
    ...
    return {
        ...
        send: async (message) => {
            const response = await postJson(hookUrl, hookToken, {
                agentId,
                message: `SYSTEM ALERT RELAY — do exactly this, nothing else. Do not investigate, do not use any other tools. ` +
            ...
            if (!response.ok)
                throw new Error(`hooks/agent relay returned ${response.status}`);
        },
    };
}
export function pushNotificationTransport() {
    ...
    return {
        ...
        send: async (message) => {
            ...
            const text = await response.text();
            if (!response.ok)
                throw new Error(`tools/invoke push_notification returned ${response.status}: ${text.slice(0, 200)}`);
            const parsed = JSON.parse(text);
            if (parsed.ok === false)
                throw new Error(`push_notification failed: ${parsed.error?.message ?? "unknown"}`);
        },
    };
}
/** Try each transport in order; resolve on first success, throw if all fail. */
export async function sendThroughChain(message, transports) {
