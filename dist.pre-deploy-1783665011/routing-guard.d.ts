...
export interface RoleGuardResult {
    ...
    correctedTo?: string;
    /** All bodies that legally fill the state's owner_role (set when blocked). */
    legalBodies?: string[];
    /**
     * AI-2044: true when the dispatch was blocked but the ticket's current
     * delegate was left untouched (delegate is legal for the state, or its
     * legality could not be verified — the guard never mutates on uncertainty).
     */
 ...
 */
export declare function fetchCurrentDelegate(issueIdOrIdentifier: string, authHeader: string): Promise<{
    ok: true;
