...
| `deploy` — host-owned deploy (`host-owned/bin/deploy-linear-connector.sh` + `systemd`/`PM2`) | `host-owned/bin/deploy-linear-connector.sh` host request-file flow + `systemctl --user restart linear-webhook-fancymatt.service` / `npm run build` + `systemd`/`PM2`/`Docker` per `docs/deployment.md` (deploy) | **Live** — shipped | Deploy/redeploy operations (existing) |
...
- Live today: identify the last known good commit on `origin/main` that passed `INF-1335`–`INF-1338` (e.g. `git ls-remote origin main` / `git log --oneline` vs `GET /health` `commit` on prod). **Limitation of the shipped surface:** `host-owned/bin/deploy-linear-connector.sh` hard-codes `DEPLOY_REF=origin/main` (line 19) and fetches/checks out `origin/main` on every run (lines 266–287) — it has no commit/ref argument and cannot redeploy an arbitrary SHA. Triggering the host request-file flow while `origin/main` is the bad release re-deploys the bad release. The `dist.pre-deploy-*` backup created at step [4/5] is retained on-host but has no shipped `restore` command — using it is manual out-of-band work outside the deployed surface. If `origin/main` is already the bad commit, rollback is **not available** via the shipped `deploy` surface — record **STOP** or explicitly accept risk in writing until Slice D (`rollback --checkpoint`) lands or `origin/main` is reverted to the known-good SHA and re-deployed via the host-owned flow (`host-owned/bin/deploy-linear-connector.sh` fetch `origin/main` + `npm run build` + `systemctl --user restart linear-webhook-fancymatt.service` health gate per `docs/deployment.md`; deploy).
...
- Detail: rollback operates on a **retained checkpoint** plus its **restore artifact** and **workflow definitions** bundle. Before restoring, it performs **verify identity** / **exact live identity** checks against the live deployment. If identity verification fails, rollback is not available and the operator must not force it. Until Slice D merges this command does not exist; live fallback via `host-owned/bin/deploy-linear-connector.sh` only deploys `origin/main` (`DEPLOY_REF=origin/main`, lines 19 and 266–287) with `systemctl --user restart linear-webhook-fancymatt.service` per `docs/deployment.md` — if `origin/main` is the bad commit, rollback is **not available** (STOP / explicitly accepted risk) until Slice D lands or `origin/main` is reverted and re-deployed.
...
# Live fallback until Slice D merges: host-owned flow only deploys origin/main (DEPLOY_REF=origin/main) via systemctl --user restart linear-webhook-fancymatt.service + build + health gate per docs/deployment.md — if origin/main is the bad commit, rollback is NOT available via shipped surface (STOP / accepted risk until revert + re-deploy or Slice D).
