...
function capabilityPolicyPath() {
    return (process.env.CAPABILITY_POLICY_PATH ??
...
const FILE_TO_TOPIC = Object.fromEntries(Object.entries(TOPIC_TO_FILE).map(([id, fn]) => [fn, id]));
function topicToFilename(topicId) {
    return TOPIC_TO_FILE[topicId] ?? `${topicId}.md`;
}
function filenameToTopicId(filename) {
    return FILE_TO_TOPIC[filename] ?? path.basename(filename, ".md");
}
// ── Topic discovery ────────────────────────────────────────────────────────
function discoverPolicyTopics() {
    const topics = [];
    const dir = policyDir();
    try {
...
function renderCapabilitiesForAgent(agentName, policy) {
    const capMap = new Map((policy.capabilities ?? []).map((c) => [c.id, c]));
    const body = policy.bodies.find((b) => b.id === agentName || b.openclaw_agent === agentName);
    if (!body)
        return null;
    const container = policy.containers.find((c) => c.id === body.container);
    if (!container)
        return null;
    const capabilities = container.grants.map((grantId) => capMap.get(grantId) ?? { id: grantId });
    return { container: container.id, capabilities };
}
// ── Auth helper ────────────────────────────────────────────────────────────
function authenticateProxyToken(req, res) {
...
export function createGuidanceRouter() {
    ...
    // GET /docs — topic list
    router.get("/", (req, res) => {
        if (!authenticateProxyToken(req, res))
            return;
    ...
    router.get("/:topic", (req, res) => {
        ...
        if (topic === "capabilities") {
            ...
            const body = `${agentName} (${rendered.container}) capabilities:\n${capList}`;
            res.json({
                topic: "capabilities",
                agent: agentName,
...
export function getDocsLiveness() {
    return { registered: true };
}
//# sourceMappingURL=guidance-router.js.map