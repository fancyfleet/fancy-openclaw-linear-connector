...
export class HoldRetryTracker {
    constructor(config) {
        ...
        this.transitionSeen = new Map();
        /** agentId → ticketId → holdAttemptCount */
        this.holdAttempts = new Map();
        this.config = {
            graceMs: config?.graceMs ?? parseEnvInt("DISPATCH_RETRY_GRACE_MS", 120000),
     ...
     */
    recordTransition(agentId, ticketId) {
     ...
     */
    incrementHoldAttempt(agentId, ticketId) {
     ...
     */
    shouldRetryHold(agentId, ticketId, dispatchAgeMs) {
        if (this.hasTransition(agentId, ticketId))
            return false;
        if (this.getHoldAttempts(agentId, ticketId) >= this.config.maxAttempts)
            return false;
        if (dispatchAgeMs !== undefined && dispatchAgeMs > this.config.graceMs)
            return false;
        return true;
    }
    /**
     ...
     */
    clearTransition(agentId, ticketId) {
     ...
     */
    clearTicket(agentId, ticketId) {
        this.transitionSeen.get(agentId)?.delete(ticketId);
        this.holdAttempts.get(agentId)?.delete(ticketId);
    }
}
//# sourceMappingURL=hold-retry-tracker.js.map