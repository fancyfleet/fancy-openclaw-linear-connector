 ...
 */
function deriveFindingId(title, description) {
...
function withStableIds(findings) {
 ...
 */
export function extractSpecFindings(description, specSource) {
    if (!description || !specSource)
        return [];
    const findings = [];
    // Section header keyed by spec_source (case-insensitive). Escape regex meta.
    const safeName = specSource.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const sectionRegex = new RegExp(`(?:#{1,4}\\s+${safeName})\\s*\\n([\\s\\S]*?)(?=\\n#{1,4}\\s|\\n*$)`, "i");
 ...
 */
export function dedupeSpawnSpec(findings, existingChildren) {
    const existingIds = new Set(existingChildren.map((c) => c.specEntryId));
    const specIds = new Set(findings.map((f) => f.id));
    const toSpawn = findings.filter((f) => !existingIds.has(f.id));
 ...
 */
export function validateFanoutSpec(description, config) {
    ...
    if (findings.length === 0) {
        return {
            ok: false,
            reason: `fan-out spec is empty or unparseable: no '${config.spec_source}' entries found in the ticket description. ` +
 ...
 */
async function createChildIssue(teamId, title, description, parentIssueId, labelIds, authToken, delegateId) {
    const mutation = `
 ...
 */
async function createBlockingRelation(blockerInternalId, blockedInternalId, authToken) {
    const mutation = `
 ...
 */
async function resolveInitialDelegate(bodyId) {
    try {
        const mod = await import("./agents.js");
        const getAgents = mod.getAgents;
 ...
 */
export async function executeFanout(parentIssueId, authToken, config, options) {
    const result = {
        created: 0,
        childIdentifiers: [],
        errors: [],
    ...
    const existingChildren = options?.existingChildren ?? (await fetchExistingSpawnChildren(parentCtx.internalId, authToken));
    ...
    if (unmatchedChildren.length > 0) {
        ...
        log.info(`fanout: ${unmatchedChildren.length} unmatched child(ren) for ${parentIssueId} ` +
    ...
    if (toSpawn.length === 0) {
        ...
        log.info(`fanout: incremental dedup — nothing new to spawn for ${parentIssueId} ` +
    ...
    if (!options?.skipPreview) {
        ...
        const previewResult = await generateSpawnPreview(parentIssueId, authToken, toSpawn.map((f) => ({ title: f.title, description: f.description })), caps);
    ...
    for (let i = 0; i < toSpawn.length; i++) {
        ...
        const child = await createChildIssue(parentCtx.teamId, childTitle, childDescription, parentInternalId, labelIds, authToken, initialDelegateId);
        if (child) {
            result.created++;
    ...
    log.info(`fanout: completed for ${parentIssueId} — ${result.created}/${toSpawn.length} children created` +
 ...
 */
async function fetchExistingSpawnChildren(parentInternalId, authToken) {
    const query = `
 ...
 */
async function postUnmatchedChildrenNote(parentInternalId, unmatched, authToken) {
    const list = unmatched.map((c) => `- ${c.identifier}${c.state ? ` (${c.state})` : ""}`).join("\n");
    const body = `**Fan-out re-entry: ${unmatched.length} unmatched child${unmatched.length === 1 ? "" : "ren"}.**\n\n` +
 ...
 */
export function shouldTriggerFanout(def, currentState, intent) {
