...
import { deliverToAgent, deliverMessageToAgent, DeliveryThrottle, DispatchDeliveryScheduler } from "./delivery/index.js";
...
let startupCommit = "unknown";
function setStartupCommit(hash) { startupCommit = hash; }
function getStartupCommit() { return startupCommit; }
/**
...
export function createApp(options) {
    ...
    const commandAuthSnapshots = new Map();
    ...
    app.get("/health", async (_req, res) => {
        ...
        const warnings = undeliverable.map((e) => {
            ...
            return {
                ...
                ticket: detail.ticket ?? e.key ?? null,
                state: detail.state ?? e.workflowState ?? null,
                delegate: detail.delegate ?? e.agent ?? null,
    ...
    async function postLinearComment(agentId, ticketId, message) {
        ...
        for (const { source, token } of tokenCandidates) {
            ...
            try {
                ...
                if (commentBody.data?.commentCreate?.comment?.id)
                    return true;
    ...
    (agentId, ticketId) => ackTracker.ensurePending(agentId, ticketId), enrolledTicketsStore, mutationAuditStore, idempotencyStore));
    ...
    const migrationAuthToken = getAccessToken("ai") ?? process.env.LINEAR_OAUTH_TOKEN ?? process.env.LINEAR_API_KEY ?? "";
    const migrationWakeFn = async (agentName, ticketIdentifier) => {
        ...
        const message = (await buildWorkflowAwareDeliveryMessage(ticketIdentifier, migrationAuthToken, actionText)) ??
            actionText;
...
if (isEntryPoint) {
    ...
    const reconciliationAuthToken = getAccessToken("ai") ?? process.env.LINEAR_OAUTH_TOKEN ?? process.env.LINEAR_API_KEY ?? "";
    const reconciliationWakeFn = async (agentName, ticketIdentifier) => {
        const sessionKey = normalizeSessionKey(ticketIdentifier);
        const agentCfg = getAgent(agentName);
        const deliveryConfig = {
            nodeBin: process.execPath,
        ...
        const message = (await buildWorkflowAwareDeliveryMessage(ticketIdentifier, reconciliationAuthToken, actionText)) ??
            actionText;
        await deliverMessageToAgent(agentName, sessionKey, message, deliveryConfig);
    };
            ...
            return rows
                ...
                if (delegate) {
                    ...
                    if (actionTimes.length > 0)
                        firstOwnerActionAtMs = Math.min(...actionTimes);
                }
                return {
        ...
        crossCheck: async (t) => {
            ...
            try {
                ...
                if (data.data === undefined)
                    return "unknown"; // auth/transport error — fail open
    ...
    app.post("/redispatch", async (req, res) => {
        if (!requireAdminSecret(req, res))
            return;
        let body;
        try {
    ...
    if (slaAuthToken) {
        const slaWakeAgent = async (identifier) => {
            ...
            const message = (await buildWorkflowAwareDeliveryMessage(identifier, slaAuthToken, actionText)) ??
                actionText;
        ...
        log.info(`AI-1773: SLA sweep cron registered (cadence=${slaCadenceMs ?? 300000}ms, store=${slaBreachStorePath}, defs=${slaWorkflowDefPath})`);
    }
    else {
