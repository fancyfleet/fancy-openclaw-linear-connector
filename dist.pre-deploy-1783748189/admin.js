...
import { mintSessionToken, verifySessionToken, sessionTokenFromRequest, setSessionCookie, clearSessionCookie, LoginRateLimiter, } from "./admin-session.js";
...
const TRIAGE_CLUSTER_KINDS = ["observations", "operational-events", "alerts"];
...
function isAuthenticated(req, expected) {
    ...
    if (secret && timingSafeEquals(secret, expected))
        return true;
...
export function createAdminRouter(deps) {
    ...
    router.get("/api/structure", async (_req, res) => {
        let workflows = [];
    ...
    router.get("/api/proposals", (_req, res) => {
        const store = deps.proposalStore;
        res.json({ proposals: store ? store.list() : [] });
    });
    router.post("/api/proposals/:id/retry-apply", async (req, res) => {
        const store = deps.proposalStore;
        if (!store) {
    ...
    router.get("/api/alerts", (req, res) => {
        ...
        const severity = severityRaw === "info" || severityRaw === "warning" || severityRaw === "critical" ? severityRaw : undefined;
    ...
    router.get("/api/dead-letters", (req, res) => {
        const store = getAlertBus().getStore();
        if (!store) {
            res.json({ items: [] });
            return;
     ...
     */
    async function loadBoardWorkflowSummaries() {
        ...
        if (dir) {
            ...
            for (const f of entries.sort()) {
                ...
                try {
                    ...
                    if (!def || !def.id || !def.states || registryIds.has(def.id))
                        continue;
                    const slaMap = new Map();
                    for (const s of def.states) {
                        slaMap.set(s.id, s.sla ? parseSlaToMs(s.sla) : null);
    ...
    router.get("/api/board", async (_req, res) => {
        ...
        for (const [wfId, summary] of summaryMap) {
            ...
            if (enrolledWorkflowIds.size > 0) {
                ...
                const hasLive = allTickets.some((t) => {
                    if (t.workflow !== wfId)
                        return false;
                    if (t.last_event_kind === 'demoted')
                        return false;
                    if (t.terminal === 1) {
                        const termAt = t.last_event_at ?? t.entered_state_at;
                        if (now - new Date(termAt).getTime() > TWENTY_FOUR_H_MS)
                            return false;
                    }
                    return true;
                });
        ...
        const ackEntriesByTicket = new Map();
        if (deps.ackTracker) {
    ...
    router.get("/api/dispatches", (_req, res) => {
    ...
    router.get("/api/board/ticket/:ticketId", (_req, res) => {
    ...
    router.get("/api/stale-digest", async (_req, res) => {
    ...
    router.get("/api/triage/clusters", (req, res) => {
        const kindRaw = typeof req.query.kind === "string" ? req.query.kind : "observations";
        if (!TRIAGE_CLUSTER_KINDS.includes(kindRaw)) {
    ...
    router.post("/api/set-state", async (req, res) => {
        ...
        const auditCommentBody = `[Admin set-state by ${invoker}] ${result.from ?? "?"} → ${targetState} — ${reason}`;
    ...
    router.post("/api/recapture-ac", async (req, res) => {
        const body = parseJsonBody(req);
        if (body === null && (Buffer.isBuffer(req.body) || typeof req.body === "string")) {
            res.status(400).json({ ok: false, error: "Malformed JSON body" });
            return;
    ...
    router.post("/api/redispatch", async (req, res) => {
        const body = parseJsonBody(req);
        if (body === null && (Buffer.isBuffer(req.body) || typeof req.body === "string")) {
        ...
        const wakeFn = async (agentId, ticketIdentifier) => {
    ...
    router.get(/^\/(?!api\/).*/, (_req, res) => {
        ...
        res
            .status(503)
            .type("html")
            .send(placeholderPage("Console UI not built", "The management console SPA is missing. Build it with: npm --prefix web install && npm --prefix web run build — the API under /admin/api/ is unaffected."));
    });
    return router;
}
