...
export function defaultDedupKey(alert) {
...
export class AlertStore {
     ...
     */
    record(alert, suppressWindowMs, now = new Date()) {
        ...
        if (latest && now.getTime() - Date.parse(String(latest.first_at)) < suppressWindowMs) {
            ...
            const row = this.db.prepare("SELECT * FROM alerts WHERE id = ?").get(Number(latest.id));
            return { row: rowToAlert(row), suppressed: true, priorBurstCount: null };
        }
        const inserted = this.db
            .prepare(`INSERT INTO alerts (first_at, last_at, severity, source, title, detail_json, agent, ticket, dedup_key)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(nowIso, nowIso, alert.severity, alert.source, alert.title, detailJson, alert.agent ?? null, alert.ticket ?? null, dedupKey);
        const row = this.db
    ...
    markPushed(id, now = new Date(), via) {
        this.db
    ...
    clusters(q = {}) {
        ...
        const where = clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
        const rows = this.db
            .prepare(`SELECT source, dedup_key, agent,
                SUM(count) AS total,
