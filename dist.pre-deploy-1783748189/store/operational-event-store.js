 ...
 * clustered.
 */
export const NON_CLUSTERING_OUTCOMES = [
    "signature-rejected",
    "duplicate",
    "dropped-stale",
...
const ERROR_OUTCOMES = new Set(["signature-rejected", "delivery-failed", "no-route", "no-activity-warn", "no-activity-failed", "deferred-at-capacity", "dispatch-undeliverable"]);
...
export class OperationalEventStore {
    ...
    clusters(query = {}) {
        ...
        const clusterParams = [...NON_CLUSTERING_OUTCOMES];
        if (query.since) {
            clusterClauses.push("occurred_at >= ?");
            clusterParams.push(query.since);
        }
        if (query.until) {
            clusterClauses.push("occurred_at <= ?");
            clusterParams.push(query.until);
        }
        const rows = this.db
            .prepare(`SELECT workflow_state, plane, outcome, COUNT(*) AS cnt,
        ...
        const excludedParams = [...NON_CLUSTERING_OUTCOMES];
        if (query.since) {
            excludedClauses.push("occurred_at >= ?");
            excludedParams.push(query.since);
        }
        if (query.until) {
            excludedClauses.push("occurred_at <= ?");
            excludedParams.push(query.until);
        }
