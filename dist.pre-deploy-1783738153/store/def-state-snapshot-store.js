 ...
 */
export function defStateSnapshotPath() {
    if (process.env.WORKFLOW_DEF_STATE_SNAPSHOT_PATH)
        return process.env.WORKFLOW_DEF_STATE_SNAPSHOT_PATH;
 ...
 */
export async function readDefStateSnapshot() {
    ...
    try {
        ...
        for (const [id, states] of Object.entries(data)) {
            if (Array.isArray(states)) {
                out[id] = states.filter((s) => typeof s === "string");
            }
        }
        return out;
    }
        ...
        const code = err?.code;
        if (code !== "ENOENT") {
            log.warn(`def-state-snapshot: failed to read snapshot at ${target} — treating as no prior version: ${err instanceof Error ? err.message : String(err)}`);
        }
        return {};
    }
}
 ...
 */
export async function writeDefStateSnapshot(snapshot) {
    const target = defStateSnapshotPath();
    try {
        ...
        log.warn(`def-state-snapshot: failed to persist snapshot to ${target}: ${err instanceof Error ? err.message : String(err)}`);
    }
}
//# sourceMappingURL=def-state-snapshot-store.js.map