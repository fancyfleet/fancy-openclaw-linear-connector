...
import { executeFanout, shouldTriggerFanout, validateFanoutSpec } from "./fanout.js";
...
async function loadDefFromFile(file) {
    ...
    if (fbWarnings.length > 0) {
        throw new Error(`Workflow definition '${def.id}' has ${fbWarnings.length} invalid fanout/barrier config(s): ${fbWarnings.join("; ")}`);
    }
    return def;
}
/**
 ...
 */
export async function getWorkflowRegistryLiveness() {
    try {
        const registry = await loadWorkflowRegistry();
        const out = {};
        for (const [id, def] of registry) {
 ...
 */
export function validateFanoutBarrierConfig(def) {
    const errors = [];
        ...
        if (state.fanout !== undefined) {
            ...
            if (fo === null || typeof fo !== "object") {
                ...
                if (typeof cfg.child_workflow !== "string" || !wfLabelPattern.test(cfg.child_workflow)) {
                    errors.push(`Workflow state '${state.id}' fanout child_workflow '${String(cfg.child_workflow)}' is not a wf:* label. ` +
 ...
 */
export async function resolveTransitionDelegate(toStateName, matchedTransition, def, issueId, cliTarget) {
 ...
 */
async function fetchGithubRepoRefs(issueId, authToken) {
 ...
 */
async function resolveTicketRepoRefs(labels, issueId, authToken) {
 ...
 */
async function postAcCaptureWarningComment(internalIssueId, issueIdentifier, authToken, cause) {
    const mutation = `
    mutation($issueId: ID!, $body: String!) {
      commentCreate(input: { issueId: $issueId, body: $body }) { success comment { id } }
    }
  `;
    const body = `[AC Capture Warning] The AC of record was **not captured** at accept time: ${cause}. ` +
...
snapshotState) {
    ...
    const currentState = typeof snapshotState === "string" && snapshotState.length > 0
        ? snapshotState
 ...
 */
export function cliVerbFor(t) {
    if (t.generic === "continue")
        return "continue-workflow";
    if (t.generic === "revision")
 ...
 */
export async function verifyCommentSatisfiedBy(issueId, commentId, authToken, callerLinearUserId) {
    ...
    try {
        ...
        const issueMatches = comment.issue?.id === issueId ||
        ...
        if (!Number.isFinite(ageMs) || ageMs > 3600000) {
...
snapshotState) {
    ...
    if (!stateNode) {
        ...
        return (`[Proxy] '${intent}' blocked: ticket is in state '${currentState}' which is not defined in workflow '${workflowId}'. ` +
    ...
    if (intent === "deploy" && !breakGlassOverride) {
        ...
        if (flagged.length > 0) {
            ...
            return (`[Proxy] 'deploy' blocked: repo '${repoList}' has no CI auto-deploy — merging alone leaves the running service on the old build, ` +
...
export async function checkRawMutationInterception(body, issueId, authToken, bodyId, callerLinearUserId, skipCommentCreate, skipLabelFields) {
    ...
    const inputDelegateId = vars?.input && typeof vars.input === "object"
        ? vars.input.delegateId
        : undefined;
    const isClearingDelegate = hasDelegateChange && (inputDelegateId === null || /\bdelegateId\s*:\s*null\b/.test(q));
    ...
    if (isClearingDelegate && delegateId && !hasStateChange) {
        ...
        return (`[Proxy] Direct delegate clear blocked: the current delegate may re-route ${issueId} ` +
    ...
    if (!stateNode) {
        ...
        log.warn(`workflow-gate: raw mutation on defunct-state ticket ${issueId} state=${currentState} wf=${workflowId} agent=${bodyId} — blocking (fail-closed, AI-1914 AC4)`);
        return (`[Proxy] Direct status/label changes are blocked on ${issueId}: state '${currentState}' no longer exists ` +
 ...
 */
async function fetchFanoutSpecDescription(issueId, authToken) {
    const query = `
 ...
 */
async function postComment(internalIssueId, body, authToken) {
    const mutation = `
    mutation($issueId: ID!, $body: String!) {
      commentCreate(input: { issueId: $issueId, body: $body }) { success comment { id } }
    }
...
export async function applyStateTransition(intent, issueId, authToken, options) {
    ...
    if (!issueId)
        return { status: "noop", code: "no-issue-id" };
    ...
    if (!workflowId)
        return { status: "noop", code: "ad-hoc" }; // ad-hoc ticket — no-op
    let def;
    try {
        const registry = await loadWorkflowRegistry();
        def = registry.get(workflowId);
    ...
    if (!def)
        return { status: "noop", code: "unknown-workflow", detail: `no definition for wf:${workflowId}` }; // unknown workflow — no-op (AI-1530)
    ...
    if (intent !== breakGlassCommand) {
        ...
        if (resolvedSourceNode?.kind === "terminal") {
            log.warn(`workflow-gate: AI-2035: terminal re-entry guard — refusing '${intent}' on ${issueId}; ` +
            ...
            return {
                ...
                detail: `'${intent}' refused: ${issueId} is already at terminal state '${resolvedSource}'. ` +
    ...
    if (fanoutConfig) {
        ...
        if (!validation.ok) {
            log.warn(`workflow-gate: AI-1992: fan-out spec REFUSED for ${issueId} (state '${currentStateName}' → '${toStateName}'): ${validation.reason}`);
            await postComment(issue.internalId, `⛔️ **Fan-out refused — transition not applied.**\n\n` +
                ...
                `No state change was made and no children were created. Fix the spec and re-run \`${intent}\`.`, authToken);
            return {
                status: "failed",
    ...
    if (matchedTransition?.capture_ac && intent === 'accept') {
        try {
            ...
            if (fetchFailed) {
                ...
                await postAcCaptureWarningComment(issue.internalId, issueId, authToken, 'description fetch failed — could not retrieve the issue description to extract acceptance criteria');
            }
            else {
                ...
                if (verbatimAc) {
                    ...
                    await postAcCaptureWarningComment(issue.internalId, issueId, authToken, "no acceptance criteria header found in the ticket description — the AC of record was not captured. Add an '## Acceptance Criteria' or '### AC' section and use the recapture verb to create the record.");
                }
            }
        }
        catch (err) {
    ...
    if (options?.delegateOverride !== undefined) {
        ...
        log.info(`workflow-gate: B2 apply: ${issueId} ${intent} — using delegateOverride=${resolvedDelegateId}, skipping resolution`);
    }
        ...
        if (isTerminal) {
            ...
            if (explicitTarget) {
                ...
                if (targetAgent?.linearUserId) {
                    ...
                    log.info(`workflow-gate: B2 apply: ${issueId} ${intent} — explicit target '${explicitTarget}' → delegate=${resolvedDelegateId}`);
                }
                else {
                    log.error(`workflow-gate: B2 apply: FAIL-CLOSED — CLI target '${explicitTarget}' cannot be resolved to a Linear user ID for ${issueId}. Register the agent in agents.json with a linearUserId. Transition aborted.`);
            ...
            if (resolvedDelegateId === undefined && wantsPriorImplementer) {
                ...
                if (priorImplementer) {
                    ...
                    if (agent?.linearUserId) {
                        ...
                        log.info(`workflow-gate: B2 apply: ${issueId} ${intent} → ${toStateName}, routing to prior implementer '${priorImplementer}'`);
                    }
                    else {
            ...
            if (resolvedDelegateId === undefined) {
                try {
                    ...
                    if (roleBodies.length === 1) {
                        ...
                        log.error(`workflow-gate: B2 apply: FAIL-CLOSED — multi-body role '${destOwnerRole}' (${roleBodies.join(", ")}) on '${intent}' for ${issueId} requires a CLI --target${wantsPriorImplementer ? " (no prior implementer recorded)" : ""} but none was supplied. Transition aborted.`);
    ...
    const implementerEntryStates = new Set();
    for (const st of def.states) {
        for (const t of st.transitions ?? []) {
            if (t.assign?.default === 'prior-implementer')
                implementerEntryStates.add(t.to);
        }
    }
    ...
    const writeOutcome = await issueUpdateAtomicVerified(issue.internalId, newLabelIds, authToken, resolvedDelegateId, resolvedNativeStateId, toStateName);
    ...
    if (applied) {
        ...
        log.error(`workflow-gate: B2 apply: transition write FAILED for ${issueId} after ${writeOutcome.attempts} attempt(s) (${writeOutcome.failureKind})` +
...
const DEFAULT_TRANSITION_WRITE_POLICY = { maxAttempts: 3, retryDelayMs: 250 };
let transitionWritePolicy = { ...DEFAULT_TRANSITION_WRITE_POLICY };
/** Test hook: override (or reset, with no args) the transition write retry policy. */
export function _setTransitionWritePolicyForTests(policy) {
 ...
 */
async function verifyTransitionWritePersisted(internalId, expected, authToken) {
 ...
 */
async function issueUpdateAtomicVerified(internalId, labelIds, authToken, delegateId, nativeStateId, expectedStateName) {
    const { maxAttempts, retryDelayMs } = transitionWritePolicy;
    let failureKind = "mutation";
    let divergent = [];
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        const verification = await verifyTransitionWritePersisted(internalId, { stateName: expectedStateName, delegateId, nativeStateId }, authToken);
...
function emitTransitionWriteFailure(args) {
    ...
    const summary = outcome.failureKind === "verification"
...
export async function setStateAtomic(ticketIdentifier, targetState, delegate, authToken, options) {
    ...
    if (def && !options?.force) {
        ...
        if (targetStateNode?.kind === "terminal" &&
            fromStateNode &&
            fromStateNode.kind !== "terminal") {
            return fail(`set-state to terminal state '${targetState}' from active workflow state '${fromState}' requires force:true`, fromState);
        }
    }
    ...
    const writeOutcome = await issueUpdateAtomicVerified(issue.internalId, newLabelIds, authToken, resolvedDelegateId, resolvedNativeStateId, targetState);
    if (!writeOutcome.ok) {
        emitTransitionWriteFailure({
 ...
 */
export async function fetchTicketVerification(issueId, authToken) {
    const { labels, delegateId } = await fetchTicketContext(issueId, authToken);
