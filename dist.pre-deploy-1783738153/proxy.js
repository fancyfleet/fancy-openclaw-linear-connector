 ...
 */
function allowMissingCliVersion() {
 ...
 */
function extractIssueIdentifier(body) {
    if (!body)
        return null;
 ...
 */
function intentToChangeTypes(intent) {
    if (!intent)
        return ["state"];
 ...
 */
function stripNullDelegateAssigneeFields(body, effectiveIntent) {
    const input = issueUpdateInput(body);
    if (!input)
        return;
...
export async function handleProxyRequest(req, res, deps) {
    ...
    const sessionKeyHeader = req.headers["x-openclaw-session-key"] ?? null;
    ...
    if (intent && isMutation) {
        ...
        const runCommand = async () => {
            ...
            let snapshotDelegateId = undefined;
            let snapshotState = undefined;
            ...
            if (effectiveIntent === "migrate-state") {
                const migrateTarget = req.headers["x-openclaw-migrate-target"] ?? null;
            ...
            const satisfiedByHeader = req.headers["x-openclaw-comment-satisfied-by"] ?? null;
            ...
            if (p3rejection) {
                ...
                const gateDeclineResponse = { errors: [{ message: p3rejection }] };
                if (issueId) {
                    try {
                        ...
                        if (wfId) {
                            ...
                            if (def) {
                                ...
                                if (currentStateName) {
                                    ...
                                    const matchedTransition = stateNode?.transitions?.find((t) => t.command === effectiveIntent);
                                    if (matchedTransition) {
                                        const resolved = await resolveTransitionDelegate(matchedTransition.to, matchedTransition, def, issueId, target ?? undefined);
                                        if (resolved !== undefined) {
                                            ...
                                            if (input) {
                                                ...
                                                log.info(`delegate-pre-resolve agent=${agentId} intent=${effectiveIntent}${ticketCtx}: injected delegateId=${resolved} into forwarded mutation`);
                                            }
                                        }
                                    }
                                }
            ...
            if (upstreamRes.ok) {
                ...
                if (deps?.mutationAuditStore && issueId) {
                    try {
                        ...
                        const auditRecords = intentToChangeTypes(effectiveIntent).map((ct) => ({
                            source: "proxy",
                ...
                try {
                    ...
                    if (effectiveIntent === "request-changes" || effectiveIntent === "reject" || effectiveIntent === "ac-fail") {
                        feedback = {
                            fromBody: req.headers["x-openclaw-from-body"] ?? null,
                            reasonCode: feedbackCategoryHeader,
                            freeText: extractCommentBody(body),
                            wakeId: req.headers["x-openclaw-wake-id"] ?? null,
                        };
                    }
                ...
                const attachTransition = transitionResult !== null &&
                ...
                if (workflowReminder || attachTransition) {
                    try {
                        ...
                        if (attachTransition)
                            parsedResponse._workflowTransition = transitionResult;
                        if (workflowReminder)
