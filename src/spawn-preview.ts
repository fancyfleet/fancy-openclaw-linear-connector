...
export const DEFAULT_SPAWN_CAPS: SpawnCaps = {
  maxChildren: 20,
  maxDepth: 3,
  approvalAbove: 10,
};
 ...
 */
export function parseSpawnCaps(): SpawnCaps {
...
export function checkCaps(
  proposedCount: number,
  currentDepth: number,
  caps: SpawnCaps = DEFAULT_SPAWN_CAPS,
): CapCheckResult {
  const result: CapCheckResult = {
    allowed: true,
    needsApproval: false,
    caps,
    depth: currentDepth,
 ...
 */
export async function generateSpawnPreview(
  parentIssueId: string,
  authToken: string,
  ...
  const proposedChildren: ProposedChild[] = findings.map((f, i) => ({
    index: i,
    title: f.title,
  ...
  log.info(
    `spawn-preview: generated preview for ${parentIssueId} — ` +
 ...
 */
export function formatPreviewComment(preview: SpawnPreview): string {
  const lines: string[] = [
 ...
 */
export function formatCapRefusalComment(capResult: CapCheckResult, parentIssueId: string): string {
  const lines: string[] = [
