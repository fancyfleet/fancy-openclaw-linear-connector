...
export async function checkWorkflowRules(
  ...
  //
  // AI-2358: Designated-approver bypass. If the matched transition also
  // specifies `requires_capability`, a caller who HOLDS that capability is
  // the designated approver and bypasses the stakes gate entirely — the
  // capability requirement is the explicit authorization. Only non-holder
  ...
  if (match.requires_human_signoff_above_stakes && def.stakes) {
    ...
    if (ticketStakesLevel >= def.stakes.threshold) {
      // AI-2358: If this transition names a requires_capability holder,
      // check whether the caller is the designated approver BEFORE blocking.
      if (match.requires_capability && isCallerKnown) {
        const isDesignatedApprover = await bodyHasCapability(bodyId, match.requires_capability);
        if (isDesignatedApprover) {
          log.info(`workflow-gate: stakes-threshold gate: ${intent} on ${issueId} — stakes level ${ticketStakesLevel} >= threshold ${def.stakes.threshold}, but caller '${bodyId}' holds '${match.requires_capability}' (designated approver) — bypassing`);
          // Fall through — designated approver bypasses the stakes gate.
        } else {
          // Has capability field but caller doesn't hold it — blocked.
          log.warn(`workflow-gate: stakes-threshold gate: ${intent} on ${issueId} blocked — stakes level ${ticketStakesLevel} >= threshold ${def.stakes.threshold}, caller '${bodyId}' is a known AI agent and does NOT hold required capability '${match.requires_capability}'`);
          return (
            `[Proxy] '${intent}' blocked: this ticket has elevated stakes (level ${ticketStakesLevel}) ` +
            `and requires human sign-off. AI agent '${bodyId}' is not the designated '${match.requires_capability}' holder. ` +
            `Use 'escape' to exit the workflow and escalate to a human, or 'reject' to send back for changes.`
          );
        }
      } else {
        // No requires_capability on the transition — original behavior: block any known AI agent.
