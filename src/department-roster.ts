...
export function departmentRosterPath(): string {
...
export function getCachedRoster(): Roster | null {
  return cachedRoster;
}
...
export async function loadRoster(): Promise<Roster | null> {
  ...
  try {
    ...
    const parsed = yaml.load(content) as Partial<Roster> | undefined | null;
    if (!parsed || typeof parsed !== "object" || !parsed.steward || !parsed.departments) {
      log.warn(
        `department-roster: roster at ${filePath} is missing steward/departments — functionary inactive`,
      );
      cachedRoster = null;
      return null;
    }
    ...
    log.debug(
      `department-roster: loaded steward='${cachedRoster.steward}' departments=[${Object.keys(cachedRoster.departments).join(", ")}] from ${filePath}`,
    );
    return cachedRoster;
  } catch (err) {
    const reason = err instanceof Error ? err.message : String(err);
    log.warn(
      `department-roster: could not load roster at ${filePath} — functionary inactive. Reason: ${reason}`,
    );
    cachedRoster = null;
    return null;
  }
...
export function resolveRoute(
  ...
  mechanical: MechanicalTarget | null | undefined,
): RouteDecision {
...
export function getRoutingFunctionaryLiveness(): {
  active: boolean;
  roster: { loaded: boolean; steward: string | null; departments: string[] };
  path: string;
} {
