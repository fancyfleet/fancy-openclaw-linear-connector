...
describe("AI-1482: Verbatim AC + Stakes-threshold sign-off", () => {
  ...
  describe("stakes-threshold gate", () => {
    ...
    // AI-2358: The stakes gate now honors the transition's requires_capability holder.
    // hanzo is the designated deploy:execute holder — they bypass the stakes gate.
    it("blocks deploy from AI agent when stakes:high (at threshold)", async () => {
      // charles does NOT hold deploy:execute, so the capability gate catches him first.
      globalThis.fetch = makeLabelFetch(["wf:dev-impl", "state:deployment", "stakes:high"]);
      const result = await checkWorkflowRules("deploy", "AI-STAKES-HIGH", "Bearer tok", "charles");
      expect(result).not.toBeNull();
      expect(result).toContain("requires the 'deploy:execute' capability");
    });

    it("allows designated deploy:execute holder (hanzo) through stakes gate on high-stakes ticket", async () => {
      // hanzo is the deployment body (has deploy:execute) AND holds the designated capability.
      // AI-2358: designated approver bypasses the stakes gate.
      globalThis.fetch = makeLabelFetch(["wf:dev-impl", "state:deployment", "stakes:high"]);
...
// The stakes-threshold gate must honor the matched transition's
// requires_capability holder. If the caller is the designated approver
// (holds the transition's required capability), the stakes gate does not
// block them — only non-holder AI agents are still blocked.

describe("checkWorkflowRules — AI-2358: stakes-threshold designated-approver bypass", () => {
  let ai2358Dir: string;
  let ai2358OriginalFetch: typeof globalThis.fetch;
  let ai2358OriginalWorkflowPath: string | undefined;
  let ai2358OriginalPolicyPath: string | undefined;

  // A sprint-like def where `approve` in `validating` has both
  // requires_human_signoff_above_stakes and requires_capability.
  const SPRINT_WITH_SIGNOFF_YAML = `
id: sprint-with-signoff
version: 1
...
`;
  ...
  // Policy with sprint:signoff capability, plus bodies that do/don't hold it.
  const SIGNOFF_POLICY_YAML = `
capabilities:
  - id: linear:transition
  - id: sprint:signoff
...
`;
  ...
  beforeAll(() => {
    ai2358OriginalWorkflowPath = process.env.WORKFLOW_DEF_PATH;
    ai2358OriginalPolicyPath = process.env.CAPABILITY_POLICY_PATH;

    ai2358Dir = fs.mkdtempSync(path.join(os.tmpdir(), "ai2358-test-"));

    const policyFile = path.join(ai2358Dir, "capability-policy.yaml");
    fs.writeFileSync(policyFile, SIGNOFF_POLICY_YAML, "utf8");
    process.env.CAPABILITY_POLICY_PATH = policyFile;

    const workflowFile = path.join(ai2358Dir, "sprint-with-signoff.yaml");
    fs.writeFileSync(workflowFile, SPRINT_WITH_SIGNOFF_YAML, "utf8");
    process.env.WORKFLOW_DEF_PATH = workflowFile;

    // Agents file
    const agentsFile = path.join(ai2358Dir, "agents.json");
    fs.writeFileSync(agentsFile, JSON.stringify({
      agents: [
        { name: "ai", linearUserId: "ai-linear-uuid", clientId: "ai-client", clientSecret: "ai-secret", accessToken: "ai-token", refreshToken: "ai-refresh" },
        { name: "astrid", linearUserId: "astrid-linear-uuid", clientId: "a-client", clientSecret: "a-secret", accessToken: "a-token", refreshToken: "a-refresh" },
        { name: "charles", linearUserId: "charles-linear-uuid", clientId: "c-client", clientSecret: "c-secret", accessToken: "c-token", refreshToken: "c-refresh" },
      ],
    }, null, 2), "utf8");
  ...
  afterAll(() => {
    if (ai2358OriginalWorkflowPath !== undefined) {
      process.env.WORKFLOW_DEF_PATH = ai2358OriginalWorkflowPath;
    } else {
      delete process.env.WORKFLOW_DEF_PATH;
    }
    if (ai2358OriginalPolicyPath !== undefined) {
      process.env.CAPABILITY_POLICY_PATH = ai2358OriginalPolicyPath;
    } else {
      delete process.env.CAPABILITY_POLICY_PATH;
    }
  });
  ...
  beforeEach(() => {
    ...
    resetPolicyCache();
    resetNativeStateCache();
    ai2358OriginalFetch = globalThis.fetch;
  });

  afterEach(() => { globalThis.fetch = ai2358OriginalFetch; });

  // AC1: Ai (holding sprint:signoff) can approve on a def with both
  // requires_human_signoff_above_stakes and requires_capability: sprint:signoff
  it("AC1: designated approver (Ai, holding sprint:signoff) bypasses stakes-threshold gate on high-stakes approve", async () => {
    globalThis.fetch = makeLabelFetch(["wf:sprint-with-signoff", "state:validating", "stakes:high"]);
    const result = await checkWorkflowRules("approve", "issue-2358-ac1", "Bearer tok", "ai");
    expect(result).toBeNull();
  });

  // AC2: Any other agent (without the capability) on the same def — should still be blocked.
  // With requires_capability: sprint:signoff on the transition, the earlier capability gate
  // catches non-holders first ("requires the 'sprint:signoff' capability").
  it("AC2: non-holder (charles, no sprint:signoff) is blocked on high-stakes approve", async () => {
    globalThis.fetch = makeLabelFetch(["wf:sprint-with-signoff", "state:validating", "stakes:high"]);
    const result = await checkWorkflowRules("approve", "issue-2358-ac2", "Bearer tok", "charles");
    expect(result).not.toBeNull();
    // charles hits the capability gate first (transition requires sprint:signoff)
    expect(result).toContain("requires the 'sprint:signoff' capability");
  });

  // AC3: Safety preserved — another body without the signoff capability is also blocked.
  it("AC3: non-holder (astrid, no sprint:signoff) is blocked on high-stakes approve", async () => {
    globalThis.fetch = makeLabelFetch(["wf:sprint-with-signoff", "state:validating", "stakes:high"]);
    const result = await checkWorkflowRules("approve", "issue-2358-ac3", "Bearer tok", "astrid");
    expect(result).not.toBeNull();
    // astrid hits the capability gate first
    expect(result).toContain("requires the 'sprint:signoff' capability");
  });

  // AC4: Low-stakes ticket — designated approver (Ai) passes through (gate not triggered).
  it("AC4: low-stakes approve passes for designated approver (gate not triggered)", async () => {
    globalThis.fetch = makeLabelFetch(["wf:sprint-with-signoff", "state:validating", "stakes:low"]);
    expect(await checkWorkflowRules("approve", "issue-2358-ac4", "Bearer tok", "ai")).toBeNull();
  });

  // AC5: Transition without requires_capability — original blocking behavior preserved.
  it("AC5: transition with no requires_capability still blocks AI agents (original behavior)", async () => {
    // We need a def where approve has requires_human_signoff but no requires_capability.
    // Use the test's own dev-impl def fixture for this — the deploy transition has
    // requires_capability: deploy:execute which IS a different capability. But deploy also
    // does not have requires_human_signoff_above_stakes. Let's test with a transition that
    // has requires_human_signoff_above_stakes but no requires_capability by using the
    // standard stakes test — the original behavior.
    // To avoid re-setting everything, write a minimal inline def for this case.

    const NO_CAP_WORKFLOW = `
id: no-cap-signoff
version: 1
entry_state: release
...
states:
  ...
  - id: done
    ...
    native_state: done
`;
    const wfFile = path.join(ai2358Dir, "no-cap-signoff.yaml");
    fs.writeFileSync(wfFile, NO_CAP_WORKFLOW, "utf8");
    process.env.WORKFLOW_DEF_PATH = wfFile;

    // Re-create the agents file to include a body that doesn't exist in SIGNOFF_POLICY
    // (ai is not in the base policy but we need it to be known — use its own agents)
    // Actually, the agents from beforeAll are already loaded. Let's reuse the policy
    // which knows `ai`, `astrid`, `charles`. ai is sprint-owner (has sprint:signoff)
    // but with no requires_capability, it should still be blocked.

    resetWorkflowCache();
    resetPolicyCache();

    globalThis.fetch = makeLabelFetch(["wf:no-cap-signoff", "state:release", "stakes:high"]);
    const result = await checkWorkflowRules("deploy", "issue-2358-ac5", "Bearer tok", "ai");
    expect(result).not.toBeNull();
    expect(result).toContain("elevated stakes");
    expect(result).toContain("cannot self-sign-off");

    // Restore the sprint-with-signoff def for subsequent tests
    process.env.WORKFLOW_DEF_PATH = path.join(ai2358Dir, "sprint-with-signoff.yaml");
    resetWorkflowCache();
  });

  // AC6: Unknown (human) caller is still allowed through — unchanged from original behavior.
  it("AC6: human/unknown caller is still allowed through the stakes-threshold gate", async () => {
    globalThis.fetch = makeLabelFetch(["wf:sprint-with-signoff", "state:validating", "stakes:high"]);
    const result = await checkWorkflowRules("approve", "issue-2358-ac6", "Bearer tok", "unknown-human");
    // unknown-human is not in the capability policy → treated as human → allowed
    expect(result).toBeNull();
  });

  // AC7: When the transition also has requires_capability, the capability gate
  // fires first — a non-holder is rejected by the capability gate before the
  // stakes gate even runs.
  it("AC7: capability gate fires first when transition has requires_capability", async () => {
    globalThis.fetch = makeLabelFetch(["wf:sprint-with-signoff", "state:validating", "stakes:high"]);
    const result = await checkWorkflowRules("approve", "issue-2358-ac7", "Bearer tok", "charles");
    expect(result).not.toBeNull();
    // charles doesn't hold sprint:signoff, so the capability gate blocks first.
    expect(result).toContain("requires the 'sprint:signoff' capability");
  });
});
