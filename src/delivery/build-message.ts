...
function buildGenericDelegationMessage(actionText: string, identifier: string, title: string): string {
  return [
    ...
    `- To hand off for review, run \`linear handoff-work ${identifier} [delegate] --comment [note]\` (use an agent delegate like Hanzo for code review, Astrid for product)`,
    ...
    "",
    "⚠️ Important: do NOT hand off to Matt Henry for review, sign-off, or closure. Route reviews to the appropriate agent (Hanzo for code review, Astrid for product, Laren for design). Only use \`needs-human\` for genuine blockers that require human action (credentials, access, approvals you cannot obtain yourself)."
  ].join("\n");
}
