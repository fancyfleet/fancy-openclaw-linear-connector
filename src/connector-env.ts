...
// INF-1330: CONNECTOR_ENV-aware runtime config. Pure function, no side effects.

export type ConnectorEnvironment = "staging" | "production";
export type DeliveryMode = "dryRun" | "live";
...
export function resolveConnectorConfig(env: Record<string, string | undefined>): ConnectorConfig {
  const environment: ConnectorEnvironment =
    env.CONNECTOR_ENV?.toLowerCase() === "staging" ? "staging" : "production";
  ...
  const webhookSecretEnvVar =
    environment === "staging" ? "LINEAR_WEBHOOK_SECRET_STAGING" : "LINEAR_WEBHOOK_SECRET";
  const deliveryDryRun = environment === "staging";
  const deliveryMode: DeliveryMode = deliveryDryRun ? "dryRun" : "live";
