...
export async function applyStateTransition(
  ...
  // ── INF-624 (backport INF-635): fan-out preflight before the parent advance ──
  // On the release line the fan-out historically ran *after* the state write and
  // fail-open, so a launch that previewed N children but created 0 still advanced
  // the parent into its barrier state (orphaned barrier → vacuous auto-advance).
  // Backported from main (86448d9): run the fan-out before the atomic transition
  // and fail closed when the preview succeeded (attempted > 0) but child creation
  // created 0. The parent stays in its current state; no transition is attempted.
  // The result is reused by the post-transition block below — executeFanout is NOT
  // re-run there, which would double-mint children.
  ...
  if (pendingFanout) {
    try {
      ...
      preTransitionFanoutResult = await executeFanout(issueId, authToken, pendingFanout.config, {
        ...
        // wake/ack contract. Carried onto the INF-624 preflight so the child
        // wake / dispatch-ack behavior that lands children as actionable is
  ...
  // INF-624 (backport INF-635): the fan-out itself ran before the parent state
  // write (preflight, above) so a zero-created create failure cannot leave the
  // parent in its barrier state. After the successful parent transition, record
  // and comment on that preflight result — the fan-out is NOT re-executed here.
