...
// ── Helpers ────────────────────────────────────────────────────────────────
function getStateLabelName(labels) {
...
function isChildTerminal(labels) {
...
async function resolveSemanticToNativeId(teamId, semanticName, authToken) {
...
async function fetchWorkflowIssues(authToken) {
    const query = `
...
async function issueUpdateState(issueId, stateId, authToken) {
    const mutation = `
...
async function issueUpdateLabelsAndState(issueId, labelIds, stateId, authToken) {
    const mutation = `
    mutation IssueUpdate($issueId: String!, $input: IssueUpdateInput!) {
      issueUpdate(id: $issueId, input: $input) {
        success
...
async function processIssue(issue, registry, authToken, result) {
    ...
    if (stateNode.native_state) {
        ...
        if (expectedId && expectedId !== issue.state.id) {
            ...
            if (healed)
                result.nativeDesyncHealed++;
            log.info(`[anti-entropy] AC1 desync ${issue.identifier}: ` +
    ...
    if (stateLabel === "managing") {
        ...
        if (!nextNativeId) {
            result.errors.push(`${issue.identifier}: barrier reconcile failed — could not resolve next native state '${nextNativeSemantic}'`);
            return;
        }
        ...
        if (reconciled)
            result.barrierMissedReconciled++;
        log.info(`[anti-entropy] AC2 barrier ${issue.identifier}: ` +
            `children=${children.length} reconciled=${reconciled}`);
    }
}
// ── Public API ─────────────────────────────────────────────────────────────
export async function runAntiEntropyPass(opts) {
    ...
    try {
        ...
        result.errors.push(`registry load failed: ${err instanceof Error ? err.message : String(err)}`);
        return result;
    }
    for (const issue of issues) {
        try {
            ...
            result.errors.push(`${issue.identifier}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    const driftFound = result.nativeDesyncFound + result.barrierMissedFound;
    if (driftFound > 0 || result.errors.length > 0) {
        log.info(`[anti-entropy] Pass: scanned=${result.scanned} ` +
    ...
    const timer = setInterval(() => {
        void (async () => {
            try {
                ...
                const driftFound = result.nativeDesyncFound + result.barrierMissedFound;
                if (driftFound > 0 || result.errors.length > 0) {
                    log.info(`[anti-entropy] Scheduled pass: scanned=${result.scanned} ` +
                        `desync=${result.nativeDesyncFound}/${result.nativeDesyncHealed} ` +
                        `barrier=${result.barrierMissedFound}/${result.barrierMissedReconciled} ` +
                        `errors=${result.errors.length}`);
                }
