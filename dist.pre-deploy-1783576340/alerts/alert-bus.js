...
const SUPPRESS_WINDOW_MS = {
    critical: 15 * 60000,
    warning: 60 * 60000,
    info: 6 * 60 * 60000,
};
const PUSH_BUDGET_WINDOW_MS = 15 * 60000;
...
function envSeverity(name, defaultVal) {
    const raw = (process.env[name] ?? "").toLowerCase();
    return raw === "info" || raw === "warning" || raw === "critical" ? raw : defaultVal;
}
async function gatewayPush(message) {
...
export class AlertBus {
    ...
    notifyInner(alert) {
        ...
        if (suppressed)
            return; // Folded into an active burst — no repeat push.
        if (!this.pushEnabled)
            return;
        if (SEVERITY_RANK[alert.severity] < SEVERITY_RANK[this.pushMinSeverity])
            return;
        ...
        if (this.pushTimestamps.length >= this.pushBudget) {
            ...
            if (!this.stormDigestSent) {
                ...
                this.sendPush(`[connector:critical] ALERT STORM — push budget (${this.pushBudget}/${PUSH_BUDGET_WINDOW_MS / 60000}min) exhausted; ` +
                    `further alerts suppressed from push. See alerts store / console for the full stream.`, null);
            }
            return;
        }
        ...
        const counts = priorBurstCount !== null ? ` (previous burst: x${priorBurstCount})` : burstCount > 1 ? ` (x${burstCount})` : "";
        const detailStr = typeof alert.detail === "string" ? alert.detail : alert.detail ? JSON.stringify(alert.detail) : "";
        const detailSnippet = detailStr ? `\n${detailStr.slice(0, 300)}` : "";
        this.sendPush(`[connector:${alert.severity}] [${alert.source}] ${alert.title}${context ? ` (${context})` : ""}${counts}${detailSnippet}`, rowId);
        this.pushTimestamps.push(nowMs);
    }
    sendPush(message, rowId) {
        this.pushFn(message)
            .then((via) => {
            if (rowId !== null && this.store)
                this.store.markPushed(rowId, this.now(), typeof via === "string" ? via : undefined);
        })
            .catch((err) => {
...
export function initAlertBus(options = {}) {
    _defaultBus = new AlertBus(options);
    return _defaultBus;
}
export function getAlertBus() {
    if (!_defaultBus)
        _defaultBus = new AlertBus();
    return _defaultBus;
}
...
export function _resetAlertBusForTests() {
    _defaultBus = null;
}
//# sourceMappingURL=alert-bus.js.map