...
export function extractAgentTarget(event) {
    ...
    if (event.type === "AgentSessionEvent") {
        const d = ("data" in event ? event.data : undefined);
 ...
 */
export function unresolvedRoutingCandidates(event) {
    if (event.type === "AgentSessionEvent")
        return [];
    const data = ("data" in event ? event.data : undefined);
    if (!data)
...
export function routeEventAll(event) {
    ...
    if (roster) {
        ...
        if (decision.reason === "department-prefix") {
            log.info(`Department route: ${event.type} identifier=${extractIssueIdentifier(event)} → ${decision.target} (prefix=${decision.matchedPrefix})`);
