...
export const APPLIED_STATE_TTL_MS = 60000;
const _store = new Map();
function normalizeKey(issueId) {
    return issueId.trim().toUpperCase();
}
/** Record the authoritative destination state for a just-applied transition. */
export function recordAppliedState(issueId, state, now = Date.now()) {
    if (!issueId)
        return;
 ...
 */
export function getAppliedState(issueId, now = Date.now()) {
    if (!issueId)
        return null;
 ...
 */
export function clearAppliedState(issueId) {
    if (!issueId)
        return;
    _store.delete(normalizeKey(issueId));
}
/** Test helper — reset the in-memory store between cases. */
export function _resetAppliedStateStore() {
    _store.clear();
}
//# sourceMappingURL=applied-state-store.js.map