...
export class DispatchIdempotencyStore {
    constructor(dbPath, dedupTtlMs) {
        this._suppressedDuplicates = 0;
        this._droppedStale = 0;
        this._delegateChangeCleared = 0;
        this._ttlExpiredAdmits = 0;
        const resolvedPath = dbPath ?? path.join(process.env.DATA_DIR ?? path.join(process.cwd(), "data"), "dispatch-idempotency.db");
     ...
     */
    checkAndRecord(ticketKey, workflowState, agent, updatedAt, options) {
        ...
        if (options?.delegateChanged) {
            ...
            const latestForAgent = this.db
                .prepare(`SELECT MAX(updated_at) as max_updated_at FROM dispatch_idempotency
           WHERE ticket_key = ? AND agent = ?`)
            ...
            this.db
                .prepare(`INSERT INTO dispatch_idempotency
        ...
        const ttlDeleteResult = this.db
            .prepare(`DELETE FROM dispatch_idempotency
        ...
        const row = this.db
            .prepare(`SELECT updated_at, created_at FROM dispatch_idempotency
         WHERE ticket_key = ? AND workflow_state = ? AND agent = ?`)
            .get(ticketKey, workflowState, agent);
        if (row) {
        ...
        this.db
            .prepare(`INSERT OR REPLACE INTO dispatch_idempotency
         (ticket_key, workflow_state, agent, updated_at, created_at)
         VALUES (?, ?, ?, ?, ?)`)
            .run(ticketKey, workflowState, agent, updatedAt, now.toISOString());
        const result = { suppressed: false, stale: false };
        if (ttlDeletedRows > 0) {
     ...
     */
    clearAgentRows(ticketKey, agent) {
...
//# sourceMappingURL=dispatch-idempotency-store.js.map