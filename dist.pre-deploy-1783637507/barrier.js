...
import { loadWorkflowDef, loadWorkflowDefById, loadWorkflowRegistry, getWorkflowId, getCurrentState } from "./workflow-gate.js";
 ...
 */
export function isBarrierState(state) {
 ...
 */
export function parseSlaToMs(sla) {
...
export async function attemptBarrierTransition(parentIdentifier, authToken, workflowDef, prefetchedParentState) {
    ...
    log.info(`barrier: ${parentIdentifier} ${currentState} → ${barrierTarget} ` +
 ...
 */
function resolveBarrierTarget(def, state) {
 ...
 */
export function isManagedBarrierFromLabels(parentLabels, defs) {
 ...
 */
export async function isManagedBarrierChild(issueIdentifier, authToken) {
    const query = `
