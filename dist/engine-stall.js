...
import { LINEAR_API_URL, resolveInternalId, } from "./linear-helpers.js";
...
export async function detectStalledChildrenWithSLA(parentIdentifier, authToken, slas) {
    ...
    for (const raw of rawStalled) {
        ...
        if (slas.length > 0) {
            ...
            log.info(`engine-stall: child ${raw.identifier} breached per-state SLA in state '${raw.currentState ?? "unknown"}' ` +
 ...
 */
async function postStallComment(internalId, body, authToken) {
    const mutation = `
 ...
 */
export function classifyStallLiveness(livenessResult) {
 ...
 */
export async function emitStallEventsWithLiveness(events, _authToken, livenessConfig) {
    const results = [];
    for (const event of events) {
        ...
        if (livenessConfig.hooksUrl) {
            try {
                const headers = {};
                if (livenessConfig.hooksToken)
                    headers["Authorization"] = livenessConfig.hooksToken;
 ...
 */
export function throttleStallRollout(events, batchSize) {
    if (batchSize <= 0)
        return { dispatch: [], deferred: [...events] };
    return {
 ...
 */
export async function triggerStallDetectionWithDedup(parentIdentifier, authToken, breachStorePath) {
