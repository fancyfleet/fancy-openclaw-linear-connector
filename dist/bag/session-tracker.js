...
export class SessionTracker {
    constructor(sessionTimeoutMs, onStaleSessions) {
        // agentId → (sessionKey → startedAt)
        this.activeSessions = new Map();
        this.pendingSignals = new Map(); // agentId → ticketIds[]
        this.sessionTimeoutMs =
            sessionTimeoutMs ??
                parseInt(process.env.SESSION_TIMEOUT_MS ?? `${DEFAULT_SESSION_TIMEOUT_MS}`, 10);
        ...
        this.cleanupTimer = setInterval(() => {
            ...
            if (staleSessions.length === 0 || !this.onStaleSessions)
                return;
     ...
     */
    startSession(agentId, sessionKey) {
    ...
    endSession(agentId, sessionKey) {
        ...
        if (sessionKey) {
            ...
            if (!had)
                return null; // That specific session wasn't tracked as active
     ...
     */
    isActiveForTicket(agentId, sessionKey) {
    ...
    queueSignal(agentId, ticketIds) {
        ...
        log.info(`Queued signal for ${agentId}: ${merged.length} ticket(s) (delivery retry)`);
    }
    /** Remove a queued pending signal, optionally across all agents. */
    removePendingTicket(ticketId, agentId) {
    ...
    getActiveSessionInfo(agentId) {
        ...
        if (!entry)
            return null;
        const [sessionKey, startedAt] = entry;
        return {
            agentId,
            sessionKey,
            startedAt,
        ...
        for (const [agentId, sessions] of this.activeSessions) {
            for (const [sessionKey, startedAt] of sessions) {
                if (now - startedAt > this.sessionTimeoutMs) {
                    log.warn(`Stale session detected for ${agentId} [${sessionKey}] (${Math.round(this.sessionTimeoutMs / 60000)}min timeout). Ending.`);
                    staleDetails.push({
                        agentId,
                        sessionKey,
                        startedAt,
