...
export async function runRescueSweep(options) {
    const { authToken, workflowRegistry = new Map(), capabilityPolicyPath, operationalEventStore, labelNameToId: injectedLabelNameToId, nowMs, } = options;
    ...
    for (const ticket of tickets) {
        ...
        if (classification === "malformed") {
            ...
            if (ticket.updatedAt) {
                ...
                if (ageMs < graceMs) {
                    log.info(`rescueMalformed: deferring ${ticket.identifier} — modified ${Math.floor(ageMs / 1000)}s ago, within grace window (${graceMs}ms); will retry on next sweep`);
                    continue;
                }
            }
