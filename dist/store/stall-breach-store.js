...
export class StallBreachStore {
    ...
    isAlreadySignaled(childId, stateEnteredAt) {
        const row = this.db
            .prepare("SELECT 1 FROM stall_breaches WHERE child_id = ? AND state_entered_at = ?")
            .get(childId, stateEnteredAt);
        return row !== undefined;
    }
    recordSignal(childId, stateEnteredAt) {
        this.db
            .prepare("INSERT OR IGNORE INTO stall_breaches (child_id, state_entered_at, signaled_at) VALUES (?, ?, ?)")
...
//# sourceMappingURL=stall-breach-store.js.map