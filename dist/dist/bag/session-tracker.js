...
export class SessionTracker {
    ...
    endSession(agentId) {
        ...
        log.info(`Session ended: ${agentId} [${session.sessionKey}] (duration: ${Math.round(duration / 1000)}s)`);
    ...
    queueSignal(agentId, ticketIds) {
        ...
        log.info(`Queued signal for ${agentId}: ${merged.length} ticket(s) (session active)`);
    }
    ...
    cleanupStale() {
        ...
        for (const [agentId, session] of this.activeSessions) {
            if (now - session.startedAt > this.sessionTimeoutMs) {
                log.warn(`Stale session detected for ${agentId} [${session.sessionKey}] (${Math.round(this.sessionTimeoutMs / 60000)}min timeout). Ending.`);
                this.activeSessions.delete(agentId);
